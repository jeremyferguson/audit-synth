...
class RunConfig:
    def __init__(
        ...
        full_csv=None,
        num_feature_selection_rounds=5,
        features_combine_count=5,
        depth=0,
        mutual_info_pool_size=2,
        output_threshold=0.5,
        predicates_per_round=5,
        use_mutual_information=False,
        manual=False,
        use_bins=False,
        eval=False,
        debug=False,
        prog_fname="program.txt",
        num_examples=10,
        examples=None,
        lang: Lang = None,
        task="image",
...
class Runner:
    def __init__(self, config):
        ...
        if not config.examples:
            ...
            all_example_fnames = list(
                all_examples[all_examples["val"] == True]["fname"]
            )

            self.user_examples = set(
                random.sample(all_example_fnames, k=config.num_examples)
            )
        else:
    ...
    def eval(self):
        ...
        for fname in self.docs:
            if fname in ground_truth["fname"].values:
                expected_val = ground_truth[ground_truth["fname"] == fname][
                    "val"
                ].array[0]
    ...
    def runAutomatic(self):
        ...
        if self.config.debug:
            print(ref_preds)
        rejected_preds = []
        for i in range(self.config.num_feature_selection_rounds):
            new_preds = self.config.heuristic(
                self.prog,
                self.docs,
                self.all_features,
                self.user_examples,
    ...
    def runManual(self):
        rejected_preds = []
        prog_complete = user_input_yn(
            f"Current program is: \n{self.prog}\nAre you satisfied with this?",
            default=False,
        )
        while not prog_complete:
            ...
            if add_preds:
                ...
                add_any = user_input_yn(
                    f"Would you like to add any of these to your program?"
                )
                if add_any:
                    for pred in new_preds:
                        self.lang.displayPred(pred)
                        add_pred = user_input_yn(
                            f"Would you like to add this predicate to your program?"
                        )
                        if add_pred:
                            self.prog.add_pred(pred)
                        else:
                    ...
                    for pred in new_preds:
                        rejected_preds.append(pred)
            prog_complete = user_input_yn(
                f"Current program is {self.prog}. Are you satisfied with this?",
                default=False,
            )
